...
import { BookOpen, Download, ExternalLink, FileText, GraduationCap, Trash2 } from 'lucide-vue-next'
...
const props = defineProps<{ resources: LearningResource[]; compact?: boolean; manageable?: boolean }>()

const emit = defineEmits<{ error: [message: string]; remove: [resource: LearningResource] }>()
...
function courseDetails(resource: LearningResource) {
  if (resource.resource_type !== 'course') return []
  ...
  if (typeof level === 'string') {
    const levelKey: Record<string, string> = {
      ...
      all_levels: 'courseLevelAll',
    }
    details.push(`${t('level')}: ${levelKey[level] ? t(levelKey[level]) : level}`)
  }
  if (typeof resource.metadata.duration === 'string') details.push(`${t('duration')}: ${resource.metadata.duration}`)
  return details
}
...
<template>
  <div class="grid w-full grid-cols-1 gap-4" :class="props.compact ? 'md:grid-cols-3' : 'lg:grid-cols-2 xl:grid-cols-3'">
    <Card v-for="resource in resources" :key="resource.id" class="flex h-full min-w-0 flex-col p-5 shadow-sm">
      ...
      <p v-if="courseDetails(resource).length" class="mt-3 text-xs text-slate-500">{{ courseDetails(resource).join(' · ') }}</p>
