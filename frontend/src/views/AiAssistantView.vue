...
async function createConversation(topic = newConversationTopic.value) {
  ...
  try {
    ...
    const conversation = await authApi.createAiConversation(auth.state.token, topic, title || undefined)
...
function startRenaming() {
  if (!activeConversation.value?.messages.length) return
...
async function deleteActiveConversation() {
  if (!auth.state.token || !activeConversation.value || sending.value || deletingConversation.value) return
  if (!window.confirm(t('deleteConversationConfirm'))) return
  const deletedId = activeConversation.value.id
  deletingConversation.value = true
  error.value = ''
  try {
    await authApi.deleteAiConversation(auth.state.token, deletedId)
    conversations.value = conversations.value.filter(item => item.id !== deletedId)
  ...
  <DashboardLayout>
    <div class="flex min-h-0 flex-col lg:h-[calc(100dvh-9.5rem)]">
      <div class="flex shrink-0 flex-col justify-between gap-4 xl:flex-row xl:items-end">
        <div><p class="text-sm font-medium text-sky-700">{{ t('workspace') }}</p><h1 class="mt-1 text-3xl font-semibold tracking-tight text-slate-900">{{ t('aiAssistant') }}</h1><p class="mt-2 max-w-3xl text-sm leading-6 text-slate-600">{{ t('aiAssistantDescription') }}</p></div>
