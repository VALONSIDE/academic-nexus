...
type SortMode = 'college' | 'score' | 'name'
...
const sort = ref<SortMode>('college')
...
function clearFilters() { search.value = ''; college.value = 'all'; sort.value = 'college' }
...
<template>
  <DashboardLayout>
    <div class="mx-auto w-full max-w-7xl">
      ...
      <Card class="mt-7 border-sky-100 bg-sky-50/60 p-5 shadow-none"><div class="flex gap-3"><span class="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-sky-600 text-white"><Sparkles class="h-4 w-4" /></span><div><p class="font-medium text-slate-900">{{ t('sameCollegeFirst') }} · {{ t('matchingAlgorithm') }}</p><p class="mt-1 text-sm leading-6 text-slate-600">{{ t('matchingAlgorithmNote') }}</p></div></div></Card>
